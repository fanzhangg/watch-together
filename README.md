...
> **Google sign-in is required in production.** `DEV_LOGIN` is off there, so
...
2. Under **Test users**, add your Google account **and your partner's**. While
   the app stays in *Testing* mode only those accounts can sign in — which is
   exactly right for a private two-person app, and avoids Google's verification
   review.
3. **Credentials → Create credentials → OAuth client ID → Web application**.
4. **Authorized JavaScript origins:** add your Render URL, e.g.
   `https://watch-together.onrender.com`. (Add `http://localhost:5173` too if you
   want Google sign-in locally.) Leave *redirect URIs* empty — we use the
   ID-token flow.
5. Copy the **Client ID** (`…apps.googleusercontent.com`). No client secret is
   needed.

> The client id is read at **runtime** from `GET /api/config`, not baked into the
> build — so you can change it with a restart, no rebuild.

### 3. Render

1. Push to GitHub, then in Render: **New → Blueprint**, point it at this repo.
   It reads [render.yaml](render.yaml) and creates the Docker web service.
2. Set the three secret env vars when prompted:
   - `DB_URL` — the Neon **pooled** string
   - `GOOGLE_CLIENT_ID` — from step 2
   - `TMDB_API_KEY` — from themoviedb.org
   
   `SESSION_SECRET` is generated by Render; `DEV_LOGIN=false` and
   `SESSION_COOKIE_SECURE=true` are already set in the blueprint. Leave them.
3. Deploy. Migrations run automatically on start.

Once it's live, go back to the Google console and make sure the Render URL is in
**Authorized JavaScript origins** — the sign-in button silently fails otherwise.

### 4. Check it

- `https://<your-app>.onrender.com/api/health` → `{"status":"ok"}`
- The login page shows **only** the Google button (no dev login).
- Sign in, create a list, add a movie, and send the invite link to your partner —
  it should be an `https://` link (Render injects `RENDER_EXTERNAL_URL`, which the
  app uses to build share links).

### Costs

~$7/mo: Render **Starter** (always-on, no cold starts) + Neon free tier.
